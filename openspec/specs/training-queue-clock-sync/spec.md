# Spec: Training Queue Clock Sync

## Purpose

Ensures the training queue countdown displayed in the frontend (`BuildingUpgradeCard.vue` and `useTrainingQueue.js`) remains accurate even when the client's clock differs from the backend's clock, by carrying the backend's current time in training order responses and using it to derive a clock offset for all remaining-time calculations.
