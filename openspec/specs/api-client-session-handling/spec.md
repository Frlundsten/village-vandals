# Spec: API Client Session Handling

## Purpose

How the frontend API client attaches credentials, transparently renews an expired access token, ends a session that cannot be renewed, and reports failures carrying the server's status and reason.
