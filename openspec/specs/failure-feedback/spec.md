# Spec: Failure Feedback

## Purpose

A failed fetch is presented to the player as a failure, never as an empty result. Covers the views that render empty states, the stores that feed them, navigation entries, header identity, and clearing per-account state on logout.
