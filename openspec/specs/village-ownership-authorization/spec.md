# Spec: Village Ownership Authorization

## Purpose

Every village-scoped endpoint proves the authenticated caller owns the village before reading or mutating it. A client-supplied `villageId` is never trusted on its own, and authorization failures surface with accurate HTTP status codes rather than as server errors.
